...
LOCALES : Locales =\
{'en': {'conda_not_activated': 'conda is not activated',
        ...
        'question_mark': '?',
        'exclamation_mark': '!',
        'help': {'install_dependency': 'choose the variant of {dependency} to install',
        ...
        'about': {'fund': 'fund ai workstation', 'subscribe': 'become a member', 'join': 'join our community'},
        'uis': {'apply_button': 'APPLY',
                ...
                'language_saved': 'Language saved. Restart the application to apply.'},
        'language_choices': {'system': 'Follow System', 'en': 'English', 'zh': '中文'},
        'choices': {'processors': {'age_modifier': 'Age Modifier',
        ...
        'uis_help': {'processors': 'How to choose: start with face_swapper for face replacement, then add face_enhancer to sharpen the result. Use deep_swapper instead when you need morph blending. Add lip_syncer for talking-head videos with audio, expression_restorer to keep the target expressions, age_modifier / face_editor for age or feature tweaks, background_remover to cut out the background, frame_colorizer for B&W footage, and frame_enhancer for whole-frame sharpness. face_debugger is for inspection only. Enable only what you need—order is top to bottom.',
                     'processors_age_modifier': 'Use when you want faces to look younger or older.',
                     'processors_background_remover': 'Use when you need to remove or replace the background.',
                     'processors_deep_swapper': 'Use for advanced face swap with morph blending; pick this or face_swapper, not both unless you know the pipeline.',
                     'processors_expression_restorer': 'Use after swap when you want to keep the target face expressions.',
                     'processors_face_debugger': 'Use only to visualize boxes/landmarks while tuning; turn off for final output.',
                     'processors_face_editor': 'Use to tweak eyes, mouth, or head pose after or instead of a simple swap.',
                     'processors_face_enhancer': 'Use after face_swapper to sharpen facial detail; lower blend if it looks fake.',
                     'processors_face_swapper': 'Default choice for putting a source face onto the target. Usually pair with face_enhancer.',
                     'processors_frame_colorizer': 'Use on black-and-white or faded video/images.',
                     'processors_frame_enhancer': 'Use to sharpen the whole frame, not only the face.',
                     'processors_lip_syncer': 'Use on videos when mouth movement should follow the audio.'}},
 'zh': {'conda_not_activated': 'conda 环境未激活',
        ...
        'finalizing_image': '正在生成最终图像，分辨率 {resolution}',
        'finalizing_image_succeeded': '生成最终图像成功',
        'finalizing_image_skipped': '已跳过最终图像生成',
        ...
        'replacing_audio_skipped': '已跳过音频替换',
        'restoring_audio_succeeded': '恢复音频成功',
        'restoring_audio_skipped': '已跳过音频恢复',
        ...
        'specify_image_or_video_output': '请指定输出图像或视频路径',
        'match_target_and_output_extension': '目标与输出文件扩展名须一致',
        ...
        'stream_not_loaded': '无法加载串流模式 {stream_mode}',
        ...
        'job_remix_step_added': '已从任务 {job_id} 混编步骤 {step_index}',
        'job_remix_step_not_added': '从任务 {job_id} 混编步骤 {step_index} 失败',
        ...
        'question_mark': '？',
        'exclamation_mark': '！',
        'help': {'install_dependency': '选择要安装的 {dependency} 变体',
                 ...
                 'source_pattern': '选择图像或音频匹配模式',
                 'target_pattern': '选择图像或视频匹配模式',
                 'output_pattern': '指定输出图像或视频匹配模式',
                 'face_detector_model': '选择负责检测人脸的模型',
                 'face_detector_size': '指定提供给人脸检测器的帧尺寸',
                 'face_detector_margin': '为画面应用上、右、下、左边距',
                 'face_detector_angles': '指定检测人脸前旋转画面的角度',
                 'face_detector_score': '按置信度过滤检测到的人脸',
                 'face_landmarker_model': '选择负责检测人脸关键点的模型',
                 'face_landmarker_score': '按置信度过滤检测到的人脸关键点',
                 'face_selector_mode': '使用基于参考脸跟踪或简单匹配',
                 'face_selector_order': '指定检测到人脸的排序方式',
                 'face_selector_age_start': '按起始年龄过滤检测到的人脸',
                 'face_selector_age_end': '按结束年龄过滤检测到的人脸',
                 'face_selector_gender': '按性别过滤检测到的人脸',
                 'face_selector_race': '按种族过滤检测到的人脸',
                 'reference_face_position': '指定用于创建参考脸的位置',
                 'reference_face_distance': '指定参考脸与目标脸的相似度阈值',
                 'reference_frame_number': '指定用于创建参考脸的帧号',
                 'face_tracker_score': '指定用于匹配跟踪人脸的重叠分数',
                 'face_occluder_model': '选择负责遮挡蒙版的模型',
                 'face_parser_model': '选择负责区域蒙版的模型',
                 'face_mask_types': '组合不同的人脸蒙版类型（可选：{choices}）',
                 'face_mask_areas': '选择区域蒙版使用的项目（可选：{choices}）',
                 'face_mask_regions': '选择区域蒙版使用的部位（可选：{choices}）',
                 'face_mask_blur': '指定框蒙版的模糊程度',
                 'face_mask_padding': '为框蒙版应用上、右、下、左内边距',
                 'voice_extractor_model': '选择负责提取人声的模型',
                 'trim_frame_start': '指定目标视频的起始帧',
                 'trim_frame_end': '指定目标视频的结束帧',
                 'temp_frame_format': '指定临时资源格式',
                 'keep_temp': '处理完成后保留临时资源',
                 'target_frame_amount': '指定转发给处理器的目标帧数量',
                 'output_image_quality': '指定图像质量（影响压缩率）',
                 'output_image_scale': '基于目标图像指定输出缩放比例',
                 'output_audio_encoder': '指定音频编码器',
                 'output_audio_quality': '指定音频质量（影响压缩率）',
                 'output_audio_volume': '基于目标视频指定音量',
                 'output_video_encoder': '指定视频编码器',
                 'output_video_preset': '在处理速度与文件大小之间权衡',
                 ...
                 'background-remover-model': '选择负责抠除背景的模型',
                 'background-remover-color': '应用背景的红、绿、蓝和透明度值',
                 'open_browser': '程序就绪后自动打开浏览器',
                 'ui_layouts': '启动一个或多个界面布局（可选：{choices}，...）',
                 'ui_workflow': '选择界面工作流',
                 'download_providers': '使用不同提供商下载（可选：{choices}，...）',
                 'download_scope': '指定下载范围',
                 'benchmark_mode': '选择基准测试模式',
                 'benchmark_resolutions': '选择基准测试分辨率（可选：{choices}，...）',
                 'benchmark_cycle_count': '指定每次基准测试的循环次数',
                 'execution_device_ids': '指定用于处理的设备',
                 'execution_providers': '使用不同推理后端（可选：{choices}，...）',
                 'execution_thread_count': '指定并行处理线程数',
                 'video_memory_strategy': '在处理速度与显存占用之间权衡',
                 'log_level': '调整终端显示的日志严重级别',
                 'halt_on_error': '发生错误时立即终止程序',
                 'run': '运行程序',
                 'headless_run': '以无界面模式运行程序',
                 'batch_run': '以批处理模式运行程序',
                 'force_download': '强制自动下载后退出',
                 'benchmark': '对程序进行基准测试',
                 'job_id': '指定任务 ID',
                 'job_status': '指定任务状态',
                 'step_index': '指定步骤索引',
                 ...
                 'job_submit_all': '提交全部草稿任务到队列',
                 'job_delete': '删除草稿、队列、失败或已完成的任务',
                 'job_delete_all': '删除全部草稿、队列、失败和已完成的任务',
                 'job_add_step': '向草稿任务添加步骤',
                 'job_remix_step': '从草稿任务混编先前步骤',
                 'job_insert_step': '向草稿任务插入步骤',
                 'job_remove_step': '从草稿任务移除步骤',
                 'job_run': '运行队列中的任务',
                 'job_run_all': '运行全部队列任务',
                 'job_retry': '重试失败的任务',
                 'job_retry_all': '重试全部失败任务'},
        'about': {'fund': '资助 AI 工作站', 'subscribe': '成为会员', 'join': '加入社区'},
        'uis': {'apply_button': '应用',
                ...
                'execution_providers_checkbox_group': '执行后端',
                ...
                'language_saved': '语言已保存，请重启应用后生效。'},
        'language_choices': {'system': '跟随系统', 'en': 'English', 'zh': '中文'},
        'choices': {'processors': {'age_modifier': '年龄修改',
                                     ...
                                     'frame_enhancer': '画面增强',
                                     'lip_syncer': '口型同步'}},
        'uis_help': {'processors': '怎么选：一般换脸先勾 face_swapper，再勾 face_enhancer 提升清晰度。需要形态融合时用 deep_swapper（通常二选一）。说话视频要口型跟音频就加 lip_syncer；想保留目标表情加 expression_restorer；改年龄用 age_modifier，微调五官/姿态用 face_editor；抠背景用 background_remover；黑白片上色用 frame_colorizer；整帧变清晰用 frame_enhancer。face_debugger 只用于调试，成片请关掉。按需勾选即可，列表从上到下依次执行。',
                     'processors_age_modifier': '想让人脸显得更年轻或更年长时勾选。',
                     'processors_background_remover': '需要抠除或替换背景时勾选。',
                     'processors_deep_swapper': '需要更深的换脸与形态融合时用；一般与 face_swapper 二选一。',
                     'processors_expression_restorer': '换脸后想保留目标脸表情时勾选。',
                     'processors_face_debugger': '仅用于查看检测框/关键点来调参；导出成片时请关闭。',
                     'processors_face_editor': '需要微调眼睛、嘴型或头部姿态时勾选。',
                     'processors_face_enhancer': '换脸后提升面部清晰度；发假就调低混合强度。',
                     'processors_face_swapper': '最常用：把源脸换到目标画面。通常再搭配 face_enhancer。',
                     'processors_frame_colorizer': '黑白或严重褪色的素材上色时勾选。',
                     'processors_frame_enhancer': '需要整帧（不只脸）更清晰时勾选。',
                     'processors_lip_syncer': '视频口型需要跟随音频时勾选。'}}}
