...
def render() -> None:
	...
	morph_label = translator.get('uis.morph_slider', MODULE_NAME)

	DEEP_SWAPPER_MODEL_LABEL_ROW, DEEP_SWAPPER_MODEL_HELP_BUTTON = help_helper.render_help_label(model_label, 'uis_help.model', MODULE_NAME, visible = has_deep_swapper)
	...
	DEEP_SWAPPER_MORPH_LABEL_ROW, DEEP_SWAPPER_MORPH_HELP_BUTTON = help_helper.render_help_label(morph_label, 'uis_help.morph', MODULE_NAME, visible = morph_visible)
...
def listen() -> None:
	DEEP_SWAPPER_MODEL_DROPDOWN.change(update_deep_swapper_model, inputs = DEEP_SWAPPER_MODEL_DROPDOWN, outputs = [ DEEP_SWAPPER_MODEL_DROPDOWN, DEEP_SWAPPER_MORPH_LABEL_ROW, DEEP_SWAPPER_MORPH_SLIDER ])
	...
	if processors_checkbox_group:
		processors_checkbox_group.change(remote_update, inputs = processors_checkbox_group, outputs = [ DEEP_SWAPPER_MODEL_LABEL_ROW, DEEP_SWAPPER_MODEL_DROPDOWN, DEEP_SWAPPER_MORPH_LABEL_ROW, DEEP_SWAPPER_MORPH_SLIDER ])
...
def remote_update(processors : List[str]) -> Tuple[gradio.Row, gradio.Dropdown, gradio.Row, gradio.Slider]:
	...
	morph_visible = has_deep_swapper and load_processor_module('deep_swapper').get_inference_pool() and load_processor_module('deep_swapper').has_morph_input()
	return gradio.Row(visible = has_deep_swapper), gradio.Dropdown(visible = has_deep_swapper), gradio.Row(visible = morph_visible), gradio.Slider(visible = morph_visible)


def update_deep_swapper_model(deep_swapper_model : DeepSwapperModel) -> Tuple[gradio.Dropdown, gradio.Row, gradio.Slider]:
	...
	if deep_swapper_module.pre_check():
		...
		return gradio.Dropdown(value = state_manager.get_item('deep_swapper_model')), gradio.Row(visible = morph_visible), gradio.Slider(visible = morph_visible)
