...
def render() -> None:
	...
	with gradio.Row():
		...
		FACE_ENHANCER_MODEL_HELP_BUTTON = help_helper.render_help_button('uis_help.model', MODULE_NAME, visible = has_face_enhancer)
	with gradio.Row():
		...
		FACE_ENHANCER_BLEND_HELP_BUTTON = help_helper.render_help_button('uis_help.blend', MODULE_NAME, visible = has_face_enhancer)
	with gradio.Row():
		...
		FACE_ENHANCER_WEIGHT_HELP_BUTTON = help_helper.render_help_button('uis_help.weight', MODULE_NAME, visible = weight_visible)
...
def listen() -> None:
	FACE_ENHANCER_MODEL_DROPDOWN.change(update_face_enhancer_model, inputs = FACE_ENHANCER_MODEL_DROPDOWN, outputs = [ FACE_ENHANCER_MODEL_DROPDOWN, FACE_ENHANCER_WEIGHT_SLIDER, FACE_ENHANCER_WEIGHT_HELP_BUTTON ])
	...
	if processors_checkbox_group:
		processors_checkbox_group.change(remote_update, inputs = processors_checkbox_group, outputs = [ FACE_ENHANCER_MODEL_DROPDOWN, FACE_ENHANCER_MODEL_HELP_BUTTON, FACE_ENHANCER_BLEND_SLIDER, FACE_ENHANCER_BLEND_HELP_BUTTON, FACE_ENHANCER_WEIGHT_SLIDER, FACE_ENHANCER_WEIGHT_HELP_BUTTON ])


def remote_update(processors : List[str]) -> Tuple[gradio.Dropdown, gradio.Button, gradio.Slider, gradio.Button, gradio.Slider, gradio.Button]:
	has_face_enhancer = 'face_enhancer' in processors
	weight_visible = has_face_enhancer and load_processor_module('face_enhancer').get_inference_pool() and load_processor_module('face_enhancer').has_weight_input()
	return gradio.Dropdown(visible = has_face_enhancer), gradio.Button(visible = has_face_enhancer), gradio.Slider(visible = has_face_enhancer), gradio.Button(visible = has_face_enhancer), gradio.Slider(visible = weight_visible), gradio.Button(visible = weight_visible)


def update_face_enhancer_model(face_enhancer_model : FaceEnhancerModel) -> Tuple[gradio.Dropdown, gradio.Slider, gradio.Button]:
	...
	if face_enhancer_module.pre_check():
		...
		return gradio.Dropdown(value = state_manager.get_item('face_enhancer_model')), gradio.Slider(visible = weight_visible), gradio.Button(visible = weight_visible)
